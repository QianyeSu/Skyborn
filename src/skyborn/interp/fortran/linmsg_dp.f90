...
contains
    ...
    ! QUICK REFERENCE
    ! PURPOSE
    !    FAST PATH FOR THE `rcm2rgrid` MISSING-ROW CLEANUP CASE:
    !    ONLY INTERIOR GAPS OF LENGTH 1 OR 2 ARE FILLED, AND LEADING /
    !    TRAILING GAPS REMAIN MISSING.
    !
    ! INPUTS / OUTPUTS
    !    X(NPTS) IS UPDATED IN PLACE USING THE SAME RESULTS AS
    ...
    subroutine dlinmsg_interior_short_gaps(x, npts, xmsg)
        ...
        do while (n <= npts - 1)
            ...
            gap_len = n - gap_start
            if (gap_start == 1 .or. n > npts .or. gap_len > 2) cycle
