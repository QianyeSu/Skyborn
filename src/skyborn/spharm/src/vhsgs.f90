...
subroutine vhsgs1(nlat, nlon, ityp, nt, imid, idvw, jdvw, v, w, mdab, &
    ...
    real, intent(in) :: br(mdab, ndab, nt), bi(mdab, ndab, nt), cr(mdab, ndab, nt), ci(mdab, ndab, nt)
    ...
    ! can make this look redundant for some cases, but the explicit initialization
    ! is easier to reason about and reduces the risk of future maintenance bugs.
    do k = 1, nt
        do j = 1, nlon
            do i = 1, idv
                ve(i, j, k) = 0.0
        ...
        do k = 1, nt
            do j = 1, nlon
                ...
                do i = 1, imm1
                    ...
                    we_val = we(i, j, k)
                    wo_val = wo(i, j, k)
                    v(i, j, k) = 0.5 * (ve_val + vo_val)
                    w(i, j, k) = 0.5 * (we_val + wo_val)
                    v(nlp1 - i, j, k) = 0.5 * (ve_val - vo_val)
                    w(nlp1 - i, j, k) = 0.5 * (we_val - wo_val)
                end do
            end do
        end do
        !$OMP END PARALLEL DO
...
subroutine vhgsi1(nlat, imid, vb, wb, dthet, dwts, dpbar, work)
    ...
    do n = 1, nlat - 1
        ...
        do i = 1, imid
            ...
            vb(i, iy) = dpbar(i, n, np) / sqrt(real(2, kind=real64) * real(n+1, kind=real64))
        end do
