...
C
      ...
      RP=R(NK)
      PP=P(NK)
      CALL CAPE_PCMIN(TP,RP,PP,T,R,P,NA,N,SIG,CAPEA,TOA,IFLAG)
      IF(IFLAG.NE.1)IFL=2
C
...
C
      ...
      CALL CAPE_PCMIN(TP,RP,PP,T,R,P,NA,N,SIG,CAPEM,TOM,IFLAG)
      IF(IFLAG.NE.1)IFL=2
C
...
C
      ...
      PP=MIN(PM,1000.0)
      RP=0.622*ES0/(PP-ES0)
      CALL CAPE_PCMIN(TP,RP,PP,T,R,P,NA,N,SIG,CAPEMS,TOMS,IFLAG)
      IF(IFLAG.NE.1)IFL=2
      RAT=SSTK/TOMS
...
C
       ...
       END
C
      SUBROUTINE CAPE_PCMIN(TP,RP,PP,T,R,P,ND,N,SIG,CAPED,TOB,IFLAG)
C
