...
subroutine calculate_potential_intensity(sst_celsius, msl_pressure_hpa, &
                                               ...
                                               v_reduc, miss_handle, outflow_source_flag)
    !---------------------------------------------------------------------------
    ! PURPOSE:
    ...
    integer, intent(in), optional :: outflow_source_flag
        ! Select how outflow diagnostics are defined:
        !   0 = Use saturated CAPE* outflow (default; matches legacy pcmin behavior)
        !   1 = Use environmental CAPE outflow from the converged eyewall parcel
    ...
    real :: to_env, otl_env                     ! Environmental outflow temp and level
    ...
    integer :: outflow_source_flag_local
    real :: v_reduc_local
    ...
    outflow_source_flag_local = 0
    if (present(outflow_source_flag)) outflow_source_flag_local = outflow_source_flag
    ...
    status_flag = 0
    to_env = missing_value
    otl_env = missing_value
    ...
    do while (abs(pnew - pmold) > pressure_tolerance)
        ...
        ! Store outflow temperature and level (lines 630-631), optionally using
        ! the environmental CAPE definition added upstream in January 2026.
        outflow_temp = toms
        outflow_level = lnbs
        if (outflow_source_flag_local == 1) then
            outflow_temp = to_env
            outflow_level = otl_env
        end if

        ! Calculate the proxy for TC efficiency (BE02, EQN. 1-3) (lines 632-636)
        ! Use multiplication instead of division
        rat = sst_kelvin * (1.0 / outflow_temp)  ! Compiler may optimize this
...
!--------------------------------------------------------------------------
! Profile-level wrapper exposing full tcpyPI-style diagnostics for Python.
!--------------------------------------------------------------------------

subroutine calculate_pi_profile_complete(sst_in, psl_in, pressure_levels, temp_in, mixing_ratio_in, &
                                         num_levels, actual_levels, max_wind, min_pressure, error_flag, &
                                         outflow_temp, outflow_level, outflow_source_flag)
    implicit none
    ...
    real, intent(in) :: temp_in(num_levels)
    real, intent(in) :: mixing_ratio_in(num_levels)
    integer, intent(in), optional :: outflow_source_flag
    ...
    real, parameter :: PA_TO_HPA = 0.01
    real, parameter :: UNDEF = -9.99e33
    real, parameter :: UNDEF = -9.99e33
    real, parameter :: K_TO_C = -273.15
    real, parameter :: KGKG_TO_GKG = 1000.0
    real :: sst_celsius, psl_hpa
    real :: temp_celsius(num_levels), mixr_gkg(num_levels)
    integer :: k, levels_to_use

    levels_to_use = min(actual_levels, num_levels)
    sst_celsius = sst_in + K_TO_C
    psl_hpa = psl_in * PA_TO_HPA

    do k = 1, num_levels
        temp_celsius(k) = temp_in(k) + K_TO_C
        mixr_gkg(k) = mixing_ratio_in(k) * KGKG_TO_GKG
    end do

    if (present(outflow_source_flag)) then
        call calculate_potential_intensity( &
            sst_celsius, psl_hpa, pressure_levels(1:levels_to_use), &
            temp_celsius(1:levels_to_use), mixr_gkg(1:levels_to_use), levels_to_use, &
            max_wind, min_pressure, outflow_temp, outflow_level, error_flag, &
...
end subroutine calculate_pi_profile_complete

!--------------------------------------------------------------------------
! Compatibility wrappers for the historical Skyborn/F2PY symbol names.
!--------------------------------------------------------------------------

subroutine calculate_pi_gridded_data(sst_in, psl_in, pressure_levels, temp_in, mixing_ratio_in, &
                                     nlat, nlon, num_levels, max_wind, min_pressure, error_flag)
    implicit none
    ...
    integer, intent(in) :: nlat, nlon, num_levels
    real, intent(in) :: sst_in(nlat, nlon)
    real, intent(in) :: psl_in(nlat, nlon)
    real, intent(in) :: pressure_levels(num_levels)
    real, intent(in) :: temp_in(num_levels, nlat, nlon)
    real, intent(in) :: mixing_ratio_in(num_levels, nlat, nlon)
    real, intent(out) :: max_wind(nlat, nlon)
    real, intent(out) :: min_pressure(nlat, nlon)
    integer, intent(out) :: error_flag
    ...
    real, parameter :: KGKG_TO_GKG = 1000.0
    real, parameter :: MISSING_VALUE = -9999.0
    real :: sst_c(nlat, nlon), psl_hpa(nlat, nlon)
    real :: temp_c(num_levels, nlat, nlon), mixr_gkg(num_levels, nlat, nlon)
    integer :: i, j, k

    do j = 1, nlon
        do i = 1, nlat
            sst_c(i, j) = sst_in(i, j) + K_TO_C
            psl_hpa(i, j) = psl_in(i, j) * PA_TO_HPA
            do k = 1, num_levels
                temp_c(k, i, j) = temp_in(k, i, j) + K_TO_C
                mixr_gkg(k, i, j) = mixing_ratio_in(k, i, j) * KGKG_TO_GKG
            end do
        end do
    end do

    call calculate_pi_3d_grid( &
        sst_c, psl_hpa, pressure_levels, temp_c, mixr_gkg, &
        nlat, nlon, num_levels, max_wind, min_pressure, MISSING_VALUE)
    error_flag = 1
...
subroutine calculate_pi_gridded_with_missing(sst_in, psl_in, pressure_levels, temp_in, mixing_ratio_in, &
    ...
    do j = 1, nlon
        do i = 1, nlat
            sst_c(i, j) = merge(MISSING_VALUE, sst_in(i, j) + K_TO_C, sst_in(i, j) == UNDEF)
            psl_hpa(i, j) = merge(MISSING_VALUE, psl_in(i, j) * PA_TO_HPA, psl_in(i, j) == UNDEF)
            do k = 1, num_levels
                temp_c(k, i, j) = merge(MISSING_VALUE, temp_in(k, i, j) + K_TO_C, temp_in(k, i, j) == UNDEF)
                mixr_gkg(k, i, j) = merge(MISSING_VALUE, mixing_ratio_in(k, i, j) * KGKG_TO_GKG, mixing_ratio_in(k, i, j) == UNDEF)
            end do
        end do
    end do

    call calculate_pi_3d_grid_with_missing( &
        sst_c, psl_hpa, pressure_levels, temp_c, mixr_gkg, &
        nlat, nlon, num_levels, max_wind, min_pressure, MISSING_VALUE)
    error_flag = 1
...
subroutine calculate_pi_single_profile(sst_in, psl_in, pressure_levels, temp_in, mixing_ratio_in, &
    ...
    call calculate_pi_profile_complete( &
        sst_in, psl_in, pressure_levels, temp_in, mixing_ratio_in, &
        num_levels, actual_levels, max_wind, min_pressure, error_flag, &
        outflow_temp, outflow_level)
end subroutine calculate_pi_single_profile

subroutine cape(parcel_temp, parcel_mixing_ratio, parcel_pressure, temp_profile, &
                mixing_ratio_profile, pressure_profile, array_size, num_points, &
                buoyancy_param, cape_value, outflow_temp, error_flag)
    implicit none

    integer, intent(in) :: array_size, num_points
    real, intent(in) :: parcel_temp, parcel_mixing_ratio, parcel_pressure
    ...
    real, intent(out) :: cape_value, outflow_temp
    integer, intent(out) :: error_flag
    real :: outflow_level

    call calculate_cape( &
        parcel_temp, parcel_mixing_ratio, parcel_pressure, temp_profile, &
        mixing_ratio_profile, pressure_profile, num_points, 0, 1, &
...
subroutine calculate_pi_4d_data(sst_in, psl_in, pressure_levels, temp_in, mixing_ratio_in, &
                                nlat, nlon, num_levels, num_times, max_wind, min_pressure, error_flag)
    implicit none
    ...
    integer, intent(in) :: nlat, nlon, num_levels, num_times
    ...
    real, parameter :: KGKG_TO_GKG = 1000.0
    real, parameter :: MISSING_VALUE = -9999.0
    real :: sst_c(num_times, nlat, nlon), psl_hpa(num_times, nlat, nlon)
    real :: temp_c(num_times, num_levels, nlat, nlon)
    real :: mixr_gkg(num_times, num_levels, nlat, nlon)
    integer :: t, i, j, k

    do j = 1, nlon
        do i = 1, nlat
            do t = 1, num_times
                sst_c(t, i, j) = sst_in(t, i, j) + K_TO_C
                psl_hpa(t, i, j) = psl_in(t, i, j) * PA_TO_HPA
                do k = 1, num_levels
                    temp_c(t, k, i, j) = temp_in(t, k, i, j) + K_TO_C
                    mixr_gkg(t, k, i, j) = mixing_ratio_in(t, k, i, j) * KGKG_TO_GKG
                end do
            end do
        end do
    end do

    call calculate_pi_4d_grid( &
        sst_c, psl_hpa, pressure_levels, temp_c, mixr_gkg, &
        num_times, nlat, nlon, num_levels, max_wind, min_pressure, MISSING_VALUE)
    error_flag = 1
...
subroutine calculate_pi_4d_with_missing(sst_in, psl_in, pressure_levels, temp_in, mixing_ratio_in, &
    ...
    do j = 1, nlon
        do i = 1, nlat
            do t = 1, num_times
                sst_c(t, i, j) = merge(MISSING_VALUE, sst_in(t, i, j) + K_TO_C, sst_in(t, i, j) == UNDEF)
                psl_hpa(t, i, j) = merge(MISSING_VALUE, psl_in(t, i, j) * PA_TO_HPA, psl_in(t, i, j) == UNDEF)
                do k = 1, num_levels
                    temp_c(t, k, i, j) = merge(MISSING_VALUE, temp_in(t, k, i, j) + K_TO_C, temp_in(t, k, i, j) == UNDEF)
                    mixr_gkg(t, k, i, j) = merge(MISSING_VALUE, mixing_ratio_in(t, k, i, j) * KGKG_TO_GKG, mixing_ratio_in(t, k, i, j) == UNDEF)
                end do
            end do
        end do
    end do

    call calculate_pi_4d_grid_with_missing( &
        sst_c, psl_hpa, pressure_levels, temp_c, mixr_gkg, &
        num_times, nlat, nlon, num_levels, max_wind, min_pressure, MISSING_VALUE)
    error_flag = 1
