! Multi-dimensional SIMD optimized geostrophic wind calculation
...
subroutine z2geouv(z, nlat, mlon, zmsg, glon, glat, ug, vg, iopt)
    ...
    real(kind=8), intent(out) :: vg(nlat, mlon) ! meridional geostrophic wind [m/s] (nlat x mlon)
...
!==============================================================================
! 3D version - shape (nlat, mlon, n3rd)
...
subroutine z2geouv_3d(z, nlat, mlon, n3rd, zmsg, glon, glat, ug, vg, iopt)
    ...
    real(kind=8), intent(out) :: vg(nlat, mlon, n3rd)  ! meridional geostrophic wind [m/s]
...
!==============================================================================
! 4D version - shape (nlat, mlon, n3rd, n4th)
! Processes multiple 2D slices (e.g., pressure levels × time steps)
!==============================================================================
subroutine z2geouv_4d(z, nlat, mlon, n3rd, n4th, zmsg, glon, glat, ug, vg, iopt)
    ...
    real(kind=8), intent(out) :: vg(nlat, mlon, n3rd, n4th)  ! meridional geostrophic wind [m/s]
...
!==============================================================================
! North-to-south data reordering routine
...
subroutine zuvnew(z, nlat, mlon, zmsg, glon, glat, ug, vg, iopt)
    ...
    integer, intent(in) :: mlon           ! number of longitudes
    integer, intent(in) :: iopt           ! longitude boundary option (0=non-cyclic, 1=cyclic)
    real(kind=8), intent(in) :: z(nlat, mlon)    ! geopotential height [gpm] (north to south)
    real(kind=8), intent(in) :: zmsg             ! missing value code
    real(kind=8), intent(in) :: glat(nlat)       ! latitudes [degrees] (north to south)
    real(kind=8), intent(in) :: glon(mlon)       ! longitudes [degrees]

    ! OUTPUT
    real(kind=8), intent(out) :: ug(nlat, mlon)  ! zonal geostrophic wind [m/s]
    real(kind=8), intent(out) :: vg(nlat, mlon)  ! meridional geostrophic wind [m/s]
...
!==============================================================================
! Core geostrophic wind calculation routine
...
subroutine z2guv(z, nlat, mlon, zmsg, glon, glat, ug, vg, iopt)
    ...
    integer, intent(in) :: mlon           ! number of longitudes
    integer, intent(in) :: iopt           ! longitude boundary option (0=non-cyclic, 1=cyclic)
    real(kind=8), intent(in) :: z(nlat, mlon)    ! geopotential height [gpm] (south to north)
    real(kind=8), intent(in) :: zmsg             ! missing value code
    real(kind=8), intent(in) :: glat(nlat)       ! latitudes [degrees] (south to north)
    real(kind=8), intent(in) :: glon(mlon)       ! longitudes [degrees]
    ...
    ! Local variables - note the boundary arrays use (nlat, mlon) indexing
    real(kind=8) :: zz(0:nlat+1, 0:mlon+1)
    ...
    ! Fill zz 'middle' - SIMD optimized - NOTE: zz uses (nlat, mlon) indexing
    do nl = 1, nlat
        !$omp simd
        do ml = 1, mlon
            zz(nl, ml) = z(nl, ml)
    ...
    ! Note: derivatives are computed using proper (nlat, mlon) indexing
    do nl = 1, nlat
        if (valid_lat(nl)) then
            !$omp simd
            do ml = 1, mlon
    ...
    if (iopt /= 1) then
        !$omp simd
        do nl = 1, nlat
            if (vg(nl, 1) /= zmsg) vg(nl, 1) = vg(nl, 1) * 2.0_8
            if (vg(nl, mlon) /= zmsg) vg(nl, mlon) = vg(nl, mlon) * 2.0_8
        end do
        !$omp end simd
    end if
